...
ImportCluster() 
{
  ...
  ERRNO=$(oc create namespace ${CLUSTER_NAME} > /dev/null 2>&1; echo $?)
  if [ $ERRNO -ne 0 ]; then
  ...
  CRDS=$(oc get secret -n ${CLUSTER_NAME} ${CLUSTER_NAME}-import -o json 2>&1)
  ERRNO=$(echo $?)
  if [ $ERRNO -ne 0 ]; then
  ...
  IMPORT=$(oc get secret -n ${CLUSTER_NAME} ${CLUSTER_NAME}-import -o json 2>&1)
  ERRNO=$(echo $?)
  if [ $ERRNO -ne 0 ]; then
